...
ActualTime = RTC.get(); //Get current time from RTC each time the Standby function is called
 ...
 {   
   if(minute(ActualTime) - previousMinute >= 15) //change the program every 15 minutes
    {
   currentProgram++; //advance program
   previousMinute = minute(ActualTime); //set to current minute on advance
