...
This sketch also includes a power on self test function to make sure everything is hooked up properly and a standby function to run when no data is received after 30 seconds. 
*/
...
#define GWTS_START_ADDRESS_ARTNET 467 //18 plus actual start channel
...
byte deltahue = 10; //for fill rainbow
...
//network addresses
byte mac[] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00}; //Set to MAC address of ethernet shield
IPAddress ip(0, 0, 0, 0); //Set to IP address of ethernet shield 
...
void loop() 
{
  ...
  {
   Standby(); //Function to display pixel patterns when not receiving E1.31 or ArtNet 
...
//Simple fill rainbow function to draw a rainbow across the pixel string
void Standby()
{
thishue--;                                                  
